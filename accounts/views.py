...
def add_employee(request):
    ...
    if request.method == 'POST':
        ...
        if not post['phoneNumber'].startswith('+254'):
            post['phoneNumber'] = "+254" + post['phoneNumber']
        request.POST = post
        ...
        if form.is_valid() and form2.is_valid() and form3.is_valid():
            ...
            messages.success(request, role + ' Account Was Created Succesfuly For ' + username)
...
def tasks(request):
    ...
    tasks = Task.objects.filter(employee=tempEmp).select_related('room_service')
