...
def current_room_services(request):
    ...
    # Fetch available food menus
    food_menus = FoodMenu.objects.filter(startDate__lte=date.today(), endDate__gte=date.today())
    ...
    context = {
        ...
        "food_menus": food_menus,  # Include food menus in context
    }

    if request.method == "POST":
        if "foodReq" in request.POST:
            menu_id = request.POST.get("foodMenu")  # Get selected food menu from form
            selected_menu = get_object_or_404(FoodMenu, id=menu_id)
            ...
            newServiceReq = RoomServices(
                ...
                room=curRoom,  
                servicesType='Food',
                foodMenu=selected_menu  # Assign selected food menu
            )
            newServiceReq.save()
        ...
        # Existing service requests (cleaningReq, techReq)...
